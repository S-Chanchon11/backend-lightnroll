...
class PreAudioProcess:
    ...
    def pitchDetector(self, hz):

        detected_note = tuner.frequencyToNote(frequency=hz)

        return detected_note
        
    def pitchDetectorDemo(self):

        note_freq_set = [330.0, 247.0, 196.0, 147.0, 110.0, 82.0]

        note_list = [tuner.frequencyToNote(frequency=x) for x in note_freq_set]
...
def main():
    ...
    note = prep_audio.pitchDetector(hz=329.0) 
